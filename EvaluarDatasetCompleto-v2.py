...
MODEL_FILE = "modelo_xgboost_v2.joblib"
INPUT_DATASET = "dataset_dga_vs_benigno_final.csv"
OUTPUT_EVALUATION = "evaluacion_resultados_modelo_v2.csv"
CHUNK_SIZE = 100000

# Diccionarios de referencia para N-gramas
COMMON_BIGRAMS = {
    'th': 0.0388,
    'he': 0.0368,
    'in': 0.0228,
...
def main():
  ...
  print(f"[*] Cargando modelo V2 desde '{MODEL_FILE}'...")
  model = joblib.load(MODEL_FILE)
  print('[+] Modelo V2 cargado exitosamente.')

  print(
      f"[*] Procesando y evaluando '{INPUT_DATASET}' con características V2..."
  )
  ...
  for chunk in pd.read_csv(INPUT_DATASET, chunksize=CHUNK_SIZE):
    ...
    features_list = [extract_features_v2(s) for s in chunk['sld']]
    ...
    chunk['clasificacion_modelo'] = [
        'MALICIOSO' if p == 1 else 'BENIGNO' for p in preds_binarias
    ]
    ...
    chunk['resultado_evaluacion'] = [
        clasificar_diagnostico(r, p)
        for r, p in zip(chunk['label'], preds_binarias)
    ]
    ...
    print(
        f'    [+] Evaluados {total_procesados:,} registros...',
        end='\r',
        flush=True,
    )
  ...
  fpr = (
      (total_fp / total_reales_negativos * 100)
      if total_reales_negativos > 0
      else 0.0
  )
  fnr = (
      (total_fn / total_reales_positivos * 100)
      if total_reales_positivos > 0
      else 0.0
  )
  precision = (
  ...
  recall = (
      (total_tp / total_reales_positivos * 100)
      if total_reales_positivos > 0
      else 0.0
  )
  accuracy = (
      ((total_tp + total_tn) / total_procesados * 100)
      if total_procesados > 0
      else 0.0
  )

  print('\n' + '=' * 70)
  print('      REPORTE ESTADÍSTICO DE EVALUACIÓN V2 (N-GRAMAS / WORDINESS)')
  print('=' * 70)
  ...
  print('-' * 70)
  print(
      f' Tasa de Falsos Positivos (FPR):       {fpr:.4f}%  (FP / Total'
      ' Benignos)'
  )
  print(
      f' Tasa de Falsos Negativos (FNR):       {fnr:.4f}%  (FN / Total DGAs)'
  )
