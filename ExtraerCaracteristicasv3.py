...
OUTPUT_FILE = "dataset_features_v3_ready.csv"
TFIDF_VECTORIZER_FILE = "tfidf_char_vectorizer.joblib"
CHUNK_SIZE = 100000

# Diccionarios base de referencia
COMMON_BIGRAMS = {
    "th": 0.0388,
    "he": 0.0368,
    "in": 0.0228,
...
def extract_wordninja_features(sld):
  """Descompone el SLD con wordninja para detectar palabras de diccionario."""
  clean_s = re.sub(r"[^a-z]", "", sld)
  if not clean_s:
...
def main():
  ...
  # 1. Ajustar TF-IDF Vectorizer
  print("[*] Entrenando vectorizador TF-IDF de caracteres (Top 50 n-gramas)...")
  df_sample = pd.read_csv(INPUT_FILE, usecols=["sld"]).dropna()
  sample_corpus = df_sample["sld"].astype(str).sample(n=200000, random_state=42)

  tfidf_vec = TfidfVectorizer(analyzer="char", ngram_range=(2, 4), max_features=50)
  tfidf_vec.fit(sample_corpus)
  joblib.dump(tfidf_vec, TFIDF_VECTORIZER_FILE)
  print(f"[+] TF-IDF guardado en '{TFIDF_VECTORIZER_FILE}'.")

  tfidf_feature_names = [f"tfidf_{name}" for name in tfidf_vec.get_feature_names_out()]

  # 2. Extracción por Chunks
  print(f"[*] Procesando '{INPUT_FILE}' con Wordninja + TF-IDF...")
  first_chunk = True
  total_processed = 0

  for chunk in pd.read_csv(INPUT_FILE, chunksize=CHUNK_SIZE):
    chunk = chunk.dropna(subset=["sld"]).copy()

    # Características base + wordninja
    base_feats = [extract_base_features(s) for s in chunk["sld"]]
    df_base = pd.DataFrame(base_feats)

    # Matriz TF-IDF
    tfidf_matrix = tfidf_vec.transform(chunk["sld"].astype(str)).toarray()
    ...
    print(f"    [+] Procesados {total_processed:,} dominios...", end="\r", flush=True)

  print(f"\n[+] Extracción V3 completada: '{OUTPUT_FILE}'\n")
